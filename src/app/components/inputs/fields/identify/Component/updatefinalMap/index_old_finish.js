...
class updateMapFiled extends Component {
  ...
  addFeatureLayersToMap() {
    ...
    var out = new esri.layers.FeatureLayer(url, {
      opacity: 0,
      id: "Landbase_Parcel_FeatureLayer",
      mode: esri.layers.FeatureLayer.MODE_ONDEMAND,
      outFields: ["*"],
    });

    //this.map.addLayer(out);

    var url_h =
      addedParcelMapServiceUrl.replace("/MapServer", "/FeatureServer") +
      "/" +
      this.LayerID.Landbase_Parcel_H +
      "?token=" +
      window.esriToken;

    var out_h = new esri.layers.FeatureLayer(url_h, {
      opacity: 0,
      id: "Landbase_Parcel_H_FeatureLayer",
      mode: esri.layers.FeatureLayer.MODE_ONDEMAND,
      outFields: ["*"],
    });

    return { landbase_fl: out, landbase_h_fl: out_h };

    // this.map.addLayer(out_h);
  }
  ...
  componentDidMount() {
    ...
    window.updateMap = function () {
      return new Promise((resolve, reject) => {
        var layers = self.addFeatureLayersToMap();

        var landbase_fb = layers.landbase_fl;
        var landbase_h_fb = layers.landbase_h_fl;
              ...
              if (res.features.length > 0) {
                res.features.forEach((f) => {
                  ...
                  f.attributes.ARCHIVE_TIME = new Date()
                    .toLocaleString()
                    .replace(",", "");
                  f.attributes.USER_NAME = "Test";
                });

                //add to history
                landbase_h_fb.applyEdits(res.features, null, null);
                ...
                self.Polygons.filter((p) => {
                  ...
                  projectedPoints.push(
                    graphic.geometry.getExtent().getCenter()
                  );
                  graphic.attributes = {
                ...
                project(projectedPoints, 4326, (projectedPointsRes) => {
                  addedFeatures.forEach((f, key) => {
                    var parcelSpatialId = GetSpatial(projectedPointsRes[key]);

                    delete res.features[0].attributes.OBJECTID;
                    res.features[0].attributes.PARCEL_AREA =
                      f.attributes.PARCEL_AREA;
                    res.features[0].attributes.PARCEL_PLAN_NO =
                      f.attributes.PARCEL_PLAN_NO;

                    f.attributes = res.features[0].attributes;
                    f.attributes.PARCEL_SPATIAL_ID = parcelSpatialId;
                  });

                  //add new parcel and delete old parcel
                  landbase_fb.applyEdits(
                    addedFeatures,
                    null,
                    null,
                    (result) => {
                      //delete parcel
                      //
                      landbase_fb.applyEdits(null, null, res.features);
                      resolve(result);
                    }
                  );
                });
