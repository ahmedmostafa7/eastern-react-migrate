    ...
    this.neighbors =
      ...
      this.neighbors.filter((f) => {
        return !this.props.mainObject?.landData?.landData?.lands?.parcels?.find(
          (p) =>
            p.attributes.PARCEL_PLAN_NO == f.attributes.PARCEL_PLAN_NO &&
            p.attributes.PLAN_NO == f.attributes.PLAN_NO
        );
      });
    ...
    if (cadDetails) {
      ...
      this.state = {
        ...
        isUnPlannedParcelIntersect:
          cadDetails?.isUnPlannedParcelIntersect || false,
        cadResults:
                      ...
                      callbackResult: (res) => {
                        ...
                        if (res.features.length) {
                          this.state.isUnPlannedParcelIntersect =
                            " ( " +
                            res.features
                              .filter((x) => x.attributes.SUBMISSIONNO)
                              .map((x) => x.attributes.SUBMISSIONNO)
                              .join(" , ") +
                            " )";
                          this.setState({
                            isUnPlannedParcelIntersect:
        ...
        {isFinishStepContractUpdate ? (
          ...
          <>
            ...
            <div>
              {this.state.isUnPlannedParcelIntersect && (
                <div>
                  الأرض المرفوعة تتقاطع مع أحد الأراضي في طبقة الأراضي الخام
                  برقم المعاملة {this.state.isUnPlannedParcelIntersect}
                </div>
              )}
              {!isView &&
                this.state.polygons &&
