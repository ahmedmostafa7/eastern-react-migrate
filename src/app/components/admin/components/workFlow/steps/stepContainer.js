...
const moveStep = (props, sourceName) => {
  ...
  const newSteps = steps.map((singleStep) => {
    return singleStep.index >= targetNo && singleStep.name !== sourceName
...
class stepContainer extends Component {
  render() {
    const { connectDragSource, connectDropTarget, isDragging, step } =
      this.props;
...
const dragable = DragSource(
  "step",
  stepSource,
  sourceCollect
)(DropTarget("step", stepTarget, targetCollect)(stepContainer));
