...
export const submitFarzSplitAndMerge = function (map, values) {
    ...
    return new Promise((resolve, reject) => {
        ...
        function toFixedArea(num, fixed) {
                        ...
                        parcelsCheckAddedBeforeRequests.push(
                            queryTask({
                                ...
                                where: "PARCEL_SPATIAL_ID = " + polygon.attributes.PARCEL_SPATIAL_ID +
                                    ...
                                    " and PARCEL_AREA like '" + toFixedArea(polygon.attributes.PARCEL_AREA, 2) + "%'",
                                outFields: ["OBJECTID"],
                    ...
                    parcelPromises.then((resultsData) => {
                        ...
                        resultsData.forEach((result, index) => {
                            ...
                            if (result.features.length == 0) {
                                ...
                                //add corners
                                addsPrepare(cornerLayer, values.layerParcels[index].corners);

                                //remove 0 length lines
                                values.layerParcels[index].lines =
                                    values.layerParcels[index].lines.filter(x => +x.attributes.BOUNDARY_LENGTH > 0.0001);
