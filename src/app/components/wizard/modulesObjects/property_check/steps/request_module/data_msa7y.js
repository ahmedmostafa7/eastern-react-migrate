...
export default {
  ...
  preSubmit(values, currentStep, props) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (
        props?.currentModule?.id == 119 &&
        props.currentModule?.record?.app_id == 27
      ) {
        resolve(values);
      } else {
        if (
            ...
            ((+parcelData.north_length).toFixed(2) !=
              ...
              (+parcelData.east_length).toFixed(2) !=
                (+(props?.formValues?.msa7yData || values?.msa7yData)
                  ?.cadDetails?.suggestionsParcels[0].data[1]
                  .totalLength).toFixed(2) ||
              (+parcelData.west_length).toFixed(2) !=
                (+(props?.formValues?.msa7yData || values?.msa7yData)
                  ?.cadDetails?.suggestionsParcels[0].data[3]
                  .totalLength).toFixed(2) ||
              (+parcelData.south_length).toFixed(2) !=
                (+(props?.formValues?.msa7yData || values?.msa7yData)
                  ?.cadDetails?.suggestionsParcels[0].data[4]
                  .totalLength).toFixed(2)) &&
            (!window.Supporting ||
          ...
          (
            ...
            Object.keys(
              ...
              props.wizardSettings.steps.data_msa7y.sections.msa7yData.fields.cadDetails.inputs[
                ...
                if (
                  !(props?.formValues?.msa7yData || values?.msa7yData)
                    ?.cadDetails.temp[input.name + index]
                ) {
                  isDescValid = false;
                  return;
                }
          ...
          if (!isDescValid) {
            window.notifySystem(
              "error",
              t("cadData:COMPLETEPARCELDESCRIPTION")
            );
            return reject();
          }
        ...
        if (valid) {
          ...
          if (mapObj) {
            ...
            mapSreenShot(mapObj, printResult, false, "data_msa7y", true);
          } else {
