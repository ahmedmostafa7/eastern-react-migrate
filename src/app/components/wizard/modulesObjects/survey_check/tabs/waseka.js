  ...
  preSubmit(values, currentStep, props) {
    return new Promise(function (resolve, reject) {
      ...
      if (
        ...
        props?.mainObject?.landData?.landData?.lands?.selectedMoamlaType == 1 &&
        tableData.find((x) => x.number_waseka.length != 12)
      ) {
        message.error(
                      ...
                      ok(values) {
                        ...
                        if (
                          props.mainObject?.landData?.landData?.lands
                            ?.selectedMoamlaType == 1
                        ) {
                          let f = data.find(
                            (d) =>
                              convertToEnglish(values.number_waseka) ==
                              d.number_waseka
                          );

                          if (f || values.selectedLands.length > 1) {
                            message.error(
                              "عذرا، لا يمكن ادخال صك واحد لأكثر من قطعة أرض"
                            );
                            new Promise.reject();
                          }
                        }
                        ...
                        if (
                          props.mainObject?.landData?.landData?.lands
                            ?.selectedMoamlaType == 2 &&
                          values.selectedLands.length !=
                            props.mainObject.landData.landData.lands.parcels
                              .length
                        ) {
                          message.error("من فضلك يجب اختيار جميع الأراضي");
                        } else {
                          if (Array.isArray(values.selectedLands)) {
                            ...
                            if (
                              ...
                              landsObj = {
                                ...values,
                                selectedLands: values.selectedLands.join(","),
                              };
                            }
