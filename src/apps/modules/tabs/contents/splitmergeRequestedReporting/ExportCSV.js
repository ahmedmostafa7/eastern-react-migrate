    ...
    >
      <ExcelSheet
        data={data
          ...
          .reduce((a, b) => {
            if (b.json_props["parcels"][0].length) {
              b.json_props["parcels"][0].forEach((parcel) => {
                a.push({
                  ...b,
              ...
              <ExcelColumn
                ...
                value={(col) => {
                  try {
                    let m = moment;
                    return (
                      convertToEnglish(eval(columns[headkey].action))
                    );
                  } catch (error) {
                    console.error(error);
                  }
