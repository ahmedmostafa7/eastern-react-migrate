...
export function arrangeSteps() {
  ...
  allSteps
    ? updateItem("/steps/updatewfsteps/", [...allSteps])
        ...
        .catch(() =>
          window.notifySystem("error", t("messages:Failed to update"))
        )
    : null;
}
    ...
    submit(values, stepsActions) {
      const index =
        allSteps && !isEmpty(allSteps)
          ...
          : 1;
      let newStep = {};
      newStep[values.name] = assign(values, { index });
      ...
      const params = {
        ...
        steps_actions: values.steps_actions
          ? values.steps_actions.map((v) =>
              find(stepsActions, { name: v }, null)
            )
          : null,
        saved_data: values.saved_data
          ? `[${getStringArray(values.saved_data)}]`
          : null,
        validate_data: values.validate_data
          ? `[${getStringArray(values.validate_data)}]`
          : null,
...
export function editStep(props) {
  const { setDialog, removeDialog, step } = props;
